...
router.post(
  ...
  async (request, response) => {
    if (!request.user)
      return response.send({ message: "You are not logged in" });
...
router.get(
  ...
  async (request, response) => {
    ...
    const favs = await user
      .find({ _id: { $in: newuser.companyfav } })
      .select("-password")
      .exec();
